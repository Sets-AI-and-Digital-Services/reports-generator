...
const SwipeContainer = ({
  ...
  const scrollByCard = (direction: "left" | "right") => {
    if (ref.current) {
      const cardWidth =
        ref.current?.firstElementChild?.clientWidth || window.innerWidth;
    ...
    <div data-no-scroll className="relative w-full">
      {!isAtStart && (
        <button
          ...
          className="absolute z-10 p-2 transform -translate-y-1/2 rounded-full left-2 top-1/2"
        >
      ...
      {!isAtEnd && (
        <button
          ...
          className="absolute z-10 p-2 transform -translate-y-1/2 rounded-full right-2 top-1/2"
        >
