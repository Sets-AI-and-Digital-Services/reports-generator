...
const TrajectoryMap: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch(
      `${API_BASE_URL}/aggregated-transitions?file_path=${selectedFile}&resolution=0.01`
    )
      .then((res) => res.json())
      .then((data) => {
    ...
    <>
      <div className="insights absolute flex flex-col z-10 text-left overflow-y-auto text-sm top-8 right-8 w-80 h-32 rounded-[11.76px] p-4 gap-2 border border-[#939598] bg-transparent backdrop-blur-[12px] shadow-[0_4px_4px_0_rgba(0,0,0,0.25)] text-white">
        ...
        <div className="flex items-start gap-2">
          ...
          <span>
            The visualization reveals the top source cities sending buses to
            Madinah, with Jeddah, Makkah, and Riyadh leading in volume.
          </span>
        </div>
        <div className="flex items-start gap-2">
          <img src={light_bulb} className="" />
        ...
        <div className="flex items-start gap-2">
          <img src={light_bulb} className="" />
          <span>
            <b>Abu Sarhad</b> demonstrates the most structured and consistent
            bus movement across the city. This reflects strong route planning,
            minimal deviation, and high operational discipline—making it a
            potential model for other operators.
          </span>
        </div>
        <div className="flex items-start gap-2">
          <img src={light_bulb} className="" />
